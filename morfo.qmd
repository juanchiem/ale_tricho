...
masa_dun <- DunnettTest(x=rhiz_tub$tub_masa, g=rhiz_tub$cepa)
masa_dun$TA %>% 
  as.data.frame %>% 
  arrange(pval) %>% 
  mutate(dif_TA_5=if_else(pval < 0.05, "si", "no")) %>% 
  mutate(dif_TA_10=if_else(pval < 0.1, "si", "no")) %>% 
  rownames_to_column("cepa")
```
